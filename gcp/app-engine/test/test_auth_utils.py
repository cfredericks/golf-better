...
def mock_secret_manager(mocker):
    mock_client = mocker.patch('auth_utils.secretmanager.SecretManagerServiceClient')
...
def test_get_gsm_secret_defaults(mock_secret_manager):
    ...
    secret = auth_utils.get_gsm_secret(secret_id)
    mock_secret_manager.access_secret_version.assert_called_once_with(name=f"projects/{auth_utils.DEFAULT_PROJECT_ID}/secrets/{secret_id}/versions/{auth_utils.DEFAULT_VERSION_ID}")
...
def test_get_gsm_secret_overrides(mock_secret_manager):
    ...
    secret = auth_utils.get_gsm_secret(secret_id, project_id, version_id)
